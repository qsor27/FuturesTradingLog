...
class Config:
    ...
    def __init__(self):
        ...
        if os.getenv('FLASK_ENV') == 'testing':
            ...
            # First try project directory, then fall back to home directory
            project_data_dir = Path(__file__).parent.parent / 'data'
            if project_data_dir.exists():
                default_data_dir = str(project_data_dir)
            else:
    ...
    def auto_import_interval(self) -> int:
        """Return auto import check interval in seconds (default 1 minute)"""
        return int(os.getenv('AUTO_IMPORT_INTERVAL', 60))
