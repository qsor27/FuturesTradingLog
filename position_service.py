...
class PositionService:
    ...
    def _calculate_position_totals_from_executions(self, position: Dict):
        ...
        # Weighted average entry price - handle None values safely
        if entries and total_entry_quantity > 0:
            weighted_entry = 0
            for e in entries:
                entry_price = e.get('entry_price', 0)
                if entry_price is not None:
                    weighted_entry += float(entry_price) * int(e.get('quantity', 0))
            position['average_entry_price'] = weighted_entry / total_entry_quantity if total_entry_quantity > 0 else 0
        else:
            position['average_entry_price'] = 0
        
        # Weighted average exit price - handle None values safely  
        if exits and total_exit_quantity > 0:
            weighted_exit = 0
            for e in exits:
                exit_price = e.get('exit_price') or e.get('entry_price', 0)
                if exit_price is not None:
                    weighted_exit += float(exit_price) * int(e.get('quantity', 0))
            position['average_exit_price'] = weighted_exit / total_exit_quantity if total_exit_quantity > 0 else 0
        else:
        ...
        # Total commission - handle None values safely
        total_commission = 0
        for e in executions:
    ...
    def _calculate_fifo_pnl(self, entries: List[Dict], exits: List[Dict], position_type: str) -> Dict:
        ...
        while entry_idx < len(sorted_entries) and exit_idx < len(sorted_exits):
            ...
            if match_qty > 0:
                entry_price_val = entry.get('entry_price', 0)
                exit_price_val = exit.get('exit_price') or exit.get('entry_price', 0)
                
                # Handle None values safely
                entry_price = float(entry_price_val) if entry_price_val is not None else 0.0
                exit_price = float(exit_price_val) if exit_price_val is not None else 0.0
