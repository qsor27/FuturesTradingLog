...
class NinjaTraderDLL:
    ...
    def _connect(self) -> bool:
        ...
        try:
            ...
            try:
                # Inspect the client object methods to see what's available
                try:
                ...
                try:
                    ...
                    try:
                        # See if TearDown is available - this should be a good test for connection
    ...
    def _get_real_time_bar(self, instrument: str) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                ...
                # Step 2: Try to access price data using the GetDouble method
                try:
                    # The logs show GetDouble is available and might be used to get price data
                    if 'GetDouble' in method_names:
                        logger.debug("Trying GetDouble method for price data")
                        try:
                            # Different NinjaTrader versions might use different arguments/indices
                            # Common indices for price data: 0=Last, 1=Bid, 2=Ask
                            # Try a few variations to see what works
                            
                            # Type 1: GetDouble(instrument, dataType)
                            # Common data types: 0=Last, 1=Bid, 2=Ask
                            try:
                                last = self.client.GetDouble(instrument_str, 0)  # Last
                                bid = self.client.GetDouble(instrument_str, 1)   # Bid
                                ask = self.client.GetDouble(instrument_str, 2)   # Ask
                                logger.debug(f"Successfully accessed price data using GetDouble")
                                has_data_methods = True
                            except Exception as e:
                                logger.debug(f"Error using GetDouble with number index: {str(e)}")
                                
                                # Type 2: Try string identifiers instead
                                try:
                                    last = self.client.GetDouble(instrument_str, "Last")
                                    bid = self.client.GetDouble(instrument_str, "Bid")
                                    ask = self.client.GetDouble(instrument_str, "Ask")
                                    logger.debug(f"Successfully accessed price data using GetDouble with string identifiers")
                                    has_data_methods = True
                                except Exception as e2:
                                    logger.debug(f"Error using GetDouble with string identifiers: {str(e2)}")
                        except Exception as get_double_error:
                            logger.debug(f"Error using GetDouble: {str(get_double_error)}")
                except Exception as ref_error:
                    logger.debug(f"Error inspecting GetDouble method: {str(ref_error)}")

                # Step 3: Try to access the client properties directly - check if they exist first
                if not has_data_methods:
                    # Check available methods using reflection
                    from System.Reflection import BindingFlags
                    client_type = self.client.GetType()
                    
                    # Check if the client class has a special format for accessing data
                    # Look for properties or fields related to price data
                    ...
                    if data_properties:
                        ...
                        try:
                            # Try accessing as simple properties first
                    ...
                    logger.debug(f"Error inspecting client: {str(ref_error)}")
                
                # Step 3: Try the methods we found in the reflection data
                if not has_data_methods:
                    try:
                        # Looking at the available methods, it seems like Ask/Bid/Last might work differently
                        # Try without parameters first (they might get active instrument data)
                        if hasattr(self.client, "Ask") and callable(getattr(self.client, "Ask")):
                            try:
                                ...
                                if all(x is not None for x in [ask, bid, last]):
                                    logger.debug("Successfully accessed price data without parameters")
                                    has_data_methods = True
                            except Exception as no_param_error:
                                logger.debug(f"Error calling without parameters: {str(no_param_error)}")
                                
                        # Try using GetDouble with string keys
                        if hasattr(self.client, "GetDouble") and callable(getattr(self.client, "GetDouble")):
                            try:
                                # Try calling with string keys
                                ask = self.client.GetDouble(String("Ask"))
                                bid = self.client.GetDouble(String("Bid"))
                                last = self.client.GetDouble(String("Last"))
                                
                                if all(x is not None for x in [ask, bid, last]):
                                    logger.debug("Successfully accessed price data using GetDouble")
                                    has_data_methods = True
                            except Exception as get_double_error:
                                logger.debug(f"Error calling GetDouble: {str(get_double_error)}")
                                
                        # Try with Playback methods if available
                        if hasattr(self.client, "AskPlayback") and callable(getattr(self.client, "AskPlayback")):
                            try:
                                # Try with and without parameters
                                ask = self.client.AskPlayback()
                                bid = self.client.BidPlayback()
                                last = self.client.LastPlayback()
                                
                                if all(x is not None for x in [ask, bid, last]):
                                    logger.debug("Successfully accessed price data using Playback methods")
                                    has_data_methods = True
                            except Exception as playback_error:
                                logger.debug(f"Error calling Playback methods: {str(playback_error)}")
                    except Exception as direct_error:
                        logger.debug(f"Error using direct method calls: {str(direct_error)}")
